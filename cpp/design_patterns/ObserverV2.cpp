...
{
public:
    Observer(unsigned long long int uid, unsigned long long int sid);
...
void BeSubscriber::member_update(unsigned long long int user_id, unsigned long long int ship_id, int chg_code, TEAM_INFO_PTR team_info)
{
    ...
    {
        auto_observer_ptr new_observer = auto_observer_ptr(new Observer(user_id, ship_id) );
        this->attach(user_id, ship_id, new_observer);
        notify_all = true;
    }
...
// 贸易 订阅仇敌通知
Observer::Observer(unsigned long long int uid, unsigned long long int sid)
{
    user_id = uid;
    ship_id = sid;
}
