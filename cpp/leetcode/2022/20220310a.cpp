...
public:
    Bank(vector<int>& balance) {
        ...
        for(int & no : balance)
        {
            cash_cache.emplace_back(no);
        }
        printVector(balance);
        printVector(cash_cache);
        // cash_cache = balance;
    }
    ...
    bool transfer(int account1, int account2, int money) {
        if(account1 < 1 || account1 > n || account2 < 1 || account2 > n )
            return false;
        if(money > cash_cache[account1-1])
            return false;
        cash_cache[account1-1] -= money;
        cash_cache[account2-1] += money;
        return true;
    }
