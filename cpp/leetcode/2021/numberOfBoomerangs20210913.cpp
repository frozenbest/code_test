...
{
    int n = chalk.size();
    long long sum = accumulate(chalk.begin(), chalk.end(), 0); // 起始,终止,累加初值
    k % sum;
    int index = -1;
    while(k >= 0) {
        index++;
        k -= chalk[index];
        std::cout<<" k= "<<k<<"index= "<<index<<"\n";
    }
    return index;
}
...
int numberOfBoomerangs(vector<vector<int>>& points) {
    ...
    for(int i = 0; i < n; ++i) {
        ...
        for(int j = 0; j < n; ++j) {
            ...
            int dis = distance(point_i,point_j);
            dis_map[dis]++;
        }
        for(auto it = dis_map.begin(); it != dis_map.end(); ++it) {
            result += it->second * (it->second-1); // 排列组合
            std::cout<<" detail. dis= "<<it->first<<" num= "<<it->second<<"\n";
        }
    }
