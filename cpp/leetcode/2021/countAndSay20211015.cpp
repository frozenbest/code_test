...
string countAndSay(int n) {
    ...
    for(int i = ans_vec.size(); i < n; ++i) {
        string& tmp = ans_vec.back();
        string ans = "";
        int count = 0;
        char last_num = ' ';
        for(int j = 0; j <= tmp.size()-1; j++) {
            if(last_num == ' ') {
                last_num = tmp[j];
                count++;
            } else if(last_num != tmp[j]) {
                ans += to_string(count) + last_num;
                last_num = tmp[j];
                count = 1;
            } else {
                count++;
