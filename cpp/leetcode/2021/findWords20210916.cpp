...
bool dfs(vector<vector<char>>& board, int x, int y, TrieNode* root, set<string>& res) {
    ...
    std::cout<<" index 2,  root->word= "<<root->word<<"\n";
    ...
    for(int i = 0; i < 4; ++i) {
        int mx = x + dis[i][0];
        int my = y + dis[i][1];
        if(mx >= 0 && mx < board.size() && my >= 0 && my < board[0].size()) { // x,y索引有效
            if(board[mx][my] != '#') {
                dfs(board, mx, my, root, res);
            }
        }
    }
    board[x][y] = ch;
    std::cout<<" index 3,  root->word= "<<root->word<<"\n";
    return true;
}
