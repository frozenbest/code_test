...
// 两个组合
int getresult2(char c1, char c2) {
    if(c1 == '*' && c2 == '*') {
    ...
    return c1 != '0' && (c1-'0')*10 + c2-'0' <= 26; // 纯数字 转化为数字跟26比较
}
...
int numDecodings(string s) {
    ...
    long long int f1 = 0, f2 = 1, fn = 0;
    // 优先算单个的 *     然后算两个整体的 +
    for(int i = 1; i <= n; i++) {

        fn = f2*getresult1(s[i-1])%mod;
        if(i > 1) {
            fn = (fn + f1*getresult2(s[i-2], s[i-1]))%mod;
        }
