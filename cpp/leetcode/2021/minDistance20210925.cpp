...
// 找出使 word1 和 word2 相同所需的最小步数 每步可删除任意一个字符串的一个字符
// 输入: "sea", "eat"
// 输出 2
...
int minDistance(string word1, string word2) {
    ...
    int dp[m+1][n+1];
    for(int i = 0; i < m+1; i++) {
        memset(dp[i], 0, (n+1)*sizeof(int)); // 初始化为0
    }
    ...
    std::cout<<" m= "<<m<<" n= "<<n<<" same_len= "<<same_len<<"\n";
    return m+n-2*same_len; // 返回不同的部分长度
}

void test() {

    int dp[9];
    iota(dp,dp+9,1); // std::iota (std::begin (data) , std::end (data) , initial); 连续递增序列填充
    for(int i = 0; i < 9; i++) {
        std::cout<<" i "<<i<<" value: "<<dp[i]<<"\n";
    }
    std::cout<<"============="<<"\n";
    ...
    for(int i = 0; i < 9; i++) {
        res ^= dp[i];
        std::cout<<" i "<<i<<" value: "<<dp[i]<<" res: "<<res<<"\n";
    }
}
...
int main(int argc, char* argv[])
{
    ...
    std::cout<<" word1= "<<word1<<" word2= "<<word2<<" res= "<<res<<"\n";

}

/*
*  g++ minDistance.cpp -o minDistance -std=c++11
*/
