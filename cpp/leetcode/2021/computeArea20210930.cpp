...
// 求两个矩形覆盖的总面积
// 每个矩形由左下(ax1,ay1)和右上顶点(ax2, ay2) 表示

int computeArea(int ax1, int ay1, int ax2, int ay2, int bx1, int by1, int bx2, int by2) {
    int ans = (ax2-ax1)*(ay2-ay1) + (bx2-bx1)*(by2-by1);

    int over_x = min(ax2, bx2) - max(ax1,bx1), over_y = min(ay2,by2) - max(ay1,by1);
    return ans - max(over_x, 0)*max(over_y, 0);
}
...
int main(int argc, char* argv[])
{
    ...
    // int res = computeArea(-2, -2, 2, 2, -2, -2, 2, 2);
    // int res = computeArea(-2, -2, 2, 2, -1, -1, 1, 1);
    int res = computeArea(-2, -2, 2, 2, 1, 1, 3, 3);
