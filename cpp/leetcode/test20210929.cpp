...
// 求二叉树节点值之和等于 targetSum的路径之和
// 路径不需要从跟节点开始 也不需要在叶子节点结束 但方向必须是向下的
int pathSum(TreeNode* root, int targetSum) {
    ...
    ret += pathSum(root->left, targetSum); // 递归每一个子节点
...
int pathSum(TreeNode* root, int targetSum, int64 sum = 0) {
    if(!root) return 0;
