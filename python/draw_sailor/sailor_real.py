...
lucky_ticket_queue = redis.StrictRedis(host="10.33.101.156",port=9014) # 加高级抽奖券 队列地址

sailor_svr_str_redis = redis.StrictRedis(host="10.33.165.240",port=16000) # 抽卡回写结果key 组6的
...
def test_single(userId):
    #add_ppwd_lucky_ticket(userId)
    drawSailorCard(userId)
